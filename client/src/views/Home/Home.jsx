...
const Home = () => {
  ...
  useEffect(() => {
    dispatch(getActivities()); // Obtener actividades y países
    if (order || filterContinent || filterActivity) {
      const filteredCountries = dispatch(
        combinedFilters(order, filterContinent, filterActivity)
      );

      // Verificar si no se encontraron países con los filtros especificados
      if (filteredCountries.length === 0) {
        // No se encontraron países, renderizar el mensaje
